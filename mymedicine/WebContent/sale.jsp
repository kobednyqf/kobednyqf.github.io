...
<table align="center" cellPadding="0" cellSpacing="1" class="query" border="1"> 
	 ...
	 <display:column title="药品" style="text-align:center" sortable="true" property="medicine_name"/> 
	  ...
	  <display:column title="出产地" style="text-align:center" sortable="true"  property="place"/> 
	   <display:column title="出售" style="text-align:center" sortable="true"  >
	   <form action="MyServlet?command=storeOut" method="post">
	   <input type="hidden" name="medicine_name" value="${store.getMedicine_name()}"> 
                <input type="hidden" name="medicine_detail" value="${store.getMedicine_detail()}">  
                <input type="hidden" name="active_time" value="${store.getActive_time()}">  
                <input type="hidden" name="inactive_time" value="${store.getInactive_time()}"> 
                <input type="hidden" name="number" value="${store.getNumber()}"> 
                <input type="hidden" name="price" value="${store.getPrice()}"> 
                <input type="hidden" name="place" value="${store.getPlace()}"> 
